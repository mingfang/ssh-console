...
public class Formatter {
    ...
    static final ThreadLocal<DateFormat> timeDF = new ThreadLocal<DateFormat>(){
        @Override
        protected DateFormat initialValue() {
            return new SimpleDateFormat("HH:mm");
        }
    };
    private final static ThreadLocal<DateFormat> timeWithSecondsDF = new ThreadLocal<DateFormat>() {
        @Override
        protected DateFormat initialValue() {
    ...
    private final static ThreadLocal<DateFormat> dateDF = new ThreadLocal<DateFormat>() {
        @Override
        protected DateFormat initialValue() {
            return new SimpleDateFormat(
                    "yyyy-MM-dd");
    ...
    static String formatClockTime(long time) {
        return timeDF.get().format(time);
    }

    static String formatDate(long time) {
        return dateDF.get().format(time);
    }

    static String formatDateTime(long time) {
        return dateDF.get().format(time) + " "
                + timeWithSecondsDF.get().format(time);
    }
    ...
    static String formatBytes(long v, long vMax, boolean html) {
        ...
        if (exp < 3) {
            ...
            s = trimDouble(v / Math.pow(10.0, 3));
        } else if (exp < 9) {
            s = trimDouble(v / Math.pow(10.0, 6));
        } else {
            s = trimDouble(v / Math.pow(10.0, 9));
        }
        if (html) {
            s = s.replace(" ", "&nbsp;");
        }
...
*/
    ...
    static String newRow(String label, String value, int columnPerRow) {
        ...
        value = "<td colspan=" + (columnPerRow - 1)
                + "> <font size =-1>" + value;

        return "<tr>" + label + value + "</tr>";
    }

    static String newRow(String label1, String value1, String label2,
                         String value2) {
